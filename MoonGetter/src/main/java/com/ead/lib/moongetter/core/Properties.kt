...
object Properties {
    ...
    const val DoodStreamIdentifier = "DoodStream"
    const val FacebookIdentifier = "Facebook"
    ...
    const val MixdropIdentifier = "Mixdrop"
    const val Mp4UploadIdentifier = "Mp4Upload"
    ...
    const val UqloadIdentifier = "Uqload"
    ...
    const val XTwitterIdentifier = "X - Twitter"
    ...
    const val DoodStream = """https://d+o+d[a-zA-Z]*\.[a-z]{2,}/e/([a-zA-Z0-9]+)"""
    const val Facebook = """https?://(www\.)?facebook\.com/(watch/\?v=(\d+)|[^/]+/videos/(\d+))"""
    ...
    const val Mixdrop =  """https://mixdrop\.[a-z]{2,}/e/([a-zA-Z0-9]+)"""
    const val Mp4upload = """https://mp4upload\.[a-z]+/embed-[\w\d]+\.html\d*"""
    ...
    const val Uqload = """https://uqload\.[a-z]+/embed-[\w\d]+\.html\d*"""
    const val Vidguard = "https?://(?:listeamed|vembed)\\.(?:com|net)/(?:e|d)/\\S+"
    const val YourUpload = """https?://.*yourupload\.com/embed/.*"""
    const val XTwitter = """https?:\/\/(?:www\.)?(?:twitter|x)\.com\/[A-Za-z0-9_]+\/status\/[0-9]+(?:\/video\/[0-9]+)?"""
}